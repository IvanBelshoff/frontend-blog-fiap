...
export const AcessoNegado: React.FC<IAcessoNegado> = ({ regras, permissoes }) => {
    ...
    return (
        <div className={theme.palette.mode == 'light' ? 'account-user-error-container' : 'account-user-error-container-dark'}>
            <Box component={Paper} variant='elevation' elevation={24} width='auto' minWidth={'40%'} height={'auto'} maxHeight={'100%'} sx={{ borderRadius: '16px', padding: 3 }} >
                ...
                <Box paddingTop={1} width='100%' display='flex' flexDirection={xsOnly || smOnly || mdOnly || lgOnly ? 'row' : 'column'} justifyContent='center' alignItems='center' gap={2}>
                    ...
                    <Box width='auto' gap={2} display='flex' flexDirection={xsOnly || smOnly || mdOnly || lgOnly ? 'column' : 'row'} justifyContent='center' alignItems='center' >
                        <Button
                            variant='contained'
                            color='primary'
                            disabled={isLoading}
                    ...
                    <Box width='100%' display='flex' flexDirection='column' justifyContent='center'>
                        ...
                        <Accordion elevation={3} expanded={open} onChange={handleOpenAccordion}>
                            ...
                            <AccordionDetails>
                                <Box width='100%' padding={2} display='flex' justifyContent='center' alignItems='center' flexDirection='column' sx={{ border: '2px solid', borderColor: theme.palette.primary.main, borderRadius: '16px' }}>

                                    <Typography variant="h6" >
                                        contate o administrador do sistema e solicite os seguintes acessos:
                                    </Typography>

                                    <Typography variant="h6" >
                                        Regras: {regras.map((regra) => (
                                            HandleformatarString(regra)
                                        ))}
                                    </Typography>
