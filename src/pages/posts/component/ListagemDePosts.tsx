import {
  ...
  import {
    IListagemDePosts,
    IResponseListagemDePostsAction
...
import { Post } from '../../post/component/Post';
  ...
  export const ListagemDePosts = () => {
    ...
    const loaderData = useLoaderData() as IListagemDePosts;
    const actionData = useActionData() as IResponseListagemDePostsAction;
    ...
    useEffect(() => {
      ...
      if (loaderData.totalCount <= 4) {
        navigate('/blog/posts?busca=&pagina=1');
      }
    ...
    return (
      ...
      >
        ...
        >
          ...
          <DialogContent>
            <DialogContentText id="alert-dialog-description" align="center">
              Deseja Realmente apagar este Post?
            </DialogContentText>
          </DialogContent>
        ...
        <div style={{ display: 'flex', textAlign: 'center' }}>
            {loaderData.data.map((post) => (<Post key={post.id} titulo={post.titulo} foto={post.foto} conteudo={post.conteudo} usuario_cadastrador={post.usuario_cadastrador} usuario_atualizador={post.usuario_atualizador} data_criacao={post.data_criacao} data_atualizacao={post.data_atualizacao}/>))}
        </div>
        {/* <TableContainer component={Paper} variant="outlined" sx={{ m: 1, width: 'auto' }}>
          <Table >
                ...
                <TableRow key={post.id}>
                  ...
                  <TableCell style={{ textAlign: 'center' }}>
                    <Tooltip title="Visualizar e editar detalhes">
                      <IconButton size="medium" onClick={() => navigate(`/blog/posts/detalhes/${pagina}/${post.id}`)}>
                        <Icon color="primary" fontSize="medium">open_in_new</Icon>
                      </IconButton>
                    </Tooltip>
                  </TableCell>
  
                  <TableCell style={{ textAlign: 'center' }}><Typography variant='body2'>{post.titulo}</Typography></TableCell>
  
                  <TableCell style={{ textAlign: 'center' }}><Typography variant='body2'>{post.conteudo}</Typography></TableCell>
  
                  <TableCell style={{ textAlign: 'center' }}><Typography variant='body2'>{post.data_criacao ? format(new Date(post.data_criacao), 'dd/MM/yyyy') : 'Desconhecido'}</Typography></TableCell>
  
                  <TableCell style={{ textAlign: 'center' }}>
  
                    <IconButton onClick={() => {
                      setUsuarioId(post.id);
                      setTipo('Dialog');
                      setOpen(true);
                    }}
                      disabled={Number(userId) == post.id ? true : false}
                    >
  
                      <Tooltip title="Deletar Post">
                        <Icon color={Number(userId) == post.id ? 'disabled' : 'primary'} >delete</Icon>
                      </Tooltip>
            ...
            <TableFooter>
              ...
              {(loaderData.totalCount > 0 && loaderData.totalCount > 10) && (
                <TableRow>
                  <TableCell colSpan={8} >
                    <Pagination
                      color="primary"
                      ...
                      page={pagina}
                      count={Math.ceil(loaderData.totalCount / 10)}
