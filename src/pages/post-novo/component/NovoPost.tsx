...
export const NovoPost = () => {
    ...
    return (
        <LayoutBaseDePagina
            titulo='Cadastre um novo computador'
            barraDeFerramentas={
                <FerramentasDeDetalhes
                    mostrarBotaoApagar={false}
                    mostrarBotaoNovo={false}
                    salvar={<Form method="POST" replace encType='multipart/form-data'>
                        <input id='titulo' type="hidden" name="titulo" value={form.titulo} />
                        <input id='conteudo' type="hidden" name="conteudo" value={form.conteudo} />
                        <input id='visivel' type="hidden" name="visivel" value={form.visivel} />
                        <input
                            style={{ display: 'none' }}
                            id="upload-photo"
                            name="foto"
                    ...
                    <Box width='100%' display='flex' flexDirection="column" justifyContent='center' alignItems='center' gap={2}>
                        ...
                        <Grid container spacing={3} justifyContent='center' >
                            ...
                            <Grid item xs={12} sm={4}>
                                <Autocomplete
                                    fullWidth
                                    isOptionEqualToValue={(option, value) => option.value === value.value} // Adicione esta linha
                                    options={[
                                    ...
                                    renderInput={(params) => (
                                        <TextField
                                            ...
                                            helperText={!!actionData?.errors?.body?.visivel && ('Visibilidade: ' + actionData?.errors?.body?.visivel)}
                                            label="Visibilidade"
                                            variant="outlined"
                                            name="visivel"
                                    ...
                                    onChange={(_, e) => { handleSelectChange(e) }}
