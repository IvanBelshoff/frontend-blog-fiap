...
export const ListagemDeUsuarios = () => {
  ...
  useEffect(() => {
    if (
      loaderData?.totalCount <= Environment.VITE_REACT_APP_LIMITE_DE_USUARIOS
    ) {
    ...
    <LayoutBaseDePagina
      ...
      barraDeFerramentas={
        <FerramentasDaListagem
          ...
          disabledBotaoNovo={Environment.validaRegraPermissaoComponents(
            JSON.parse(regras || ""),
            [Environment.REGRAS.REGRA_USUARIO],
            JSON.parse(permissoes || ""),
            [Environment.PERMISSOES.PERMISSAO_CRIAR_USUARIO]
          )}
          textoDaBusca={busca}
    ...
    >
      ...
      >
        <Table>
          <TableHead>
            <TableRow>
              ...
              {Environment.validaRegraPermissaoComponentsMetodos(
                JSON.parse(regras || ""),
                [Environment.REGRAS.REGRA_ADMIN]
              ) && (
                  ...
                  <TableCell style={{ textAlign: "center" }}>
                    <Tooltip title="Visualizar e editar detalhes">
                      <IconButton
                        size="medium"
                        onClick={() =>
                          navigate(`detalhes/${pagina}/${usuario.id}`)
                        }
                      >
                  ...
                  {Environment.validaRegraPermissaoComponentsMetodos(
                    ...
                    <TableCell style={{ textAlign: "center" }}>
                      <IconButton
                        ...
                        onClick={() =>
                          navigate(
                            `detalhes/${pagina}/${usuario.id}/regras/permissoes`
                          )
                        }
                      >
          ...
          {(loaderData?.totalCount && loaderData.totalCount) === 0 &&
            !isLoading && <caption>{Environment.LISTAGEM_VAZIA}</caption>}
          <TableFooter>
            ...
            {loaderData.totalCount > 0 &&
              loaderData.totalCount >
                Environment.VITE_REACT_APP_LIMITE_DE_USUARIOS && (
                <TableRow>
                  <TableCell colSpan={8}>
                    <Pagination
                      color="primary"
                      ...
                      onChange={(_, newPage) =>
                        handleSearchParams(undefined, newPage.toString())
                      }
                    />
                  </TableCell>
                </TableRow>
