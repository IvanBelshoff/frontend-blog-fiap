...
import {
  IDetalhesDeUsuarioAction,
  IDetalhesDeUsuarioLoader,
  IFormUsuario,
...
export const DetalhesDeUsuario = () => {
  ...
  const initialForm: IFormUsuario = {
    ...
    bloqueado:
      loaderData?.usuario?.bloqueado == true ? "true" : "false" || "false",
    senha: "",
  ...
  const [typeSeverity, setTypeSeverity] = useState<"success" | "error">(
    "success"
  );
  const [tipo, setTipo] = useState<"Dialog" | "Snackbar" | "">("");
  const [actionDataDeleteUsuario, setActionDataDeleteUsuario] =
    useState<IDetalhesDeUsuarioAction>(fetcher.data);
  ...
  const handleRemoveImage = () => {
    ...
    if (document.getElementById("upload-photo")) {
      ...
      setUploadedImage(
        originalImage || `${Environment.BASE_URL}/profile/profile.jpg`
      );
    }
    setStatePhoto("original");
  };
  ...
  const handleDisabledPassWord = () => {
    if (
      ...
      form.sobrenome != initialForm.sobrenome ||
  ...
  if (
    location.pathname ==
    String(`/blog/usuarios/detalhes/${pagina}/${id}/regras/permissoes`)
  ) {
    return <Outlet />;
  }
  ...
  return (
    <LayoutBaseDePagina
      ...
      barraDeFerramentas={
        <FerramentasDeDetalhes
          mostrarBotaoApagar={Environment.validaRegraPermissaoComponentsMetodos(
            JSON.parse(regras || ""),
            [Environment.REGRAS.REGRA_USUARIO],
            JSON.parse(permissoes || ""),
            [Environment.PERMISSOES.PERMISSAO_DELETAR_USUARIO]
          )}
                ...
                <Grid container spacing={3} justifyContent="center">
                  ...
                  <Grid item xl={4} xs={12} md={6} sm={6}>
                    <TextField
                      ...
                      value={format(
                        new Date(form.data_atualizacao),
                        "yyyy-MM-dd"
                      )}
                      type="date"
                    />
